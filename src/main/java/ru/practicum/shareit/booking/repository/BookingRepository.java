...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    List<Booking> getAllWaitingUsersItemsBookings(Long userId, LocalDateTime startDateTime, BookingStatus bookingStatus);

    @Query("select b from Booking b Inner join Item i on b.item.id = i.id where i.owner.id = ?1 " +
            "and b.status = ?2 order by b.start desc")
    List<Booking> getAllRejectedUsersItemsBookings(Long userId, BookingStatus bookingStatus);

    @Query("select b from Booking b Inner join Item i on b.item.id = i.id where i.owner.id = ?1 " +
            "and ?2 between b.start and b.end order by b.start desc")
    List<Booking> getAllCurrentUsersItemsBookings(Long userId, LocalDateTime endDateTime);

    @Query("select b from Booking b Inner join Item i on b.item.id = i.id where i.owner.id = ?1 " +
            "and b.end < ?2 order by b.start desc")
    List<Booking> getAllPastUsersItemsBookings(Long userId, LocalDateTime startDateTime);

    @Query("select b from Booking b Inner join Item i on b.item.id = i.id where i.owner.id = ?1 " +
            "and b.start > ?2 order by b.start desc")
    List<Booking> getAllFutureUsersItemsBookings(Long userId, LocalDateTime startDateTime, BookingStatus bookingStatus);
